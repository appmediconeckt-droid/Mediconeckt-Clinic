...
const Sidebar = ({ menuItemClick, collapsed = false, isSidebarOpen = false, toggleSidebar }) => {
  ...
  // Determine visual state class: mobile open > hover-expanded > collapsed
  let stateClass = "";
  if (isSidebarOpen) stateClass = "open";
  else if (hoverExpand) stateClass = "hover-expanded";
  else if (collapsed) stateClass = "collapsed";
  ...
  return (
    <>
      ...
      <div
        className={`sidebar-container ${stateClass} ${getThemeClass()}`}
        onMouseEnter={() => {
          // On large screens, if sidebar is collapsed, hovering should expand it
          if (collapsed && !isSidebarOpen) setHoverExpand(true);
        }}
        onMouseLeave={() => {
          if (collapsed && !isSidebarOpen) setHoverExpand(false);
        }}
      >
      <div className="sidebar">
        ...
        {/* Footer (optional) - always visible in expanded / open state */}
        {/* {(!collapsed || hoverExpand || isSidebarOpen) && (
